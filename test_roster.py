...
class TestGetFightRoster(unittest.TestCase):
    def test_filters_to_players_present_in_this_fight(self):
        # A third player exists report-wide but wasn't in THIS pull.
        actors_with_sub = dict(ACTORS)
        actors_with_sub[20] = Actor(id=20, name="Benchwarmer", type="Player", subtype="Mage")

        fight = Fight(
            id=1, name="Test Boss", difficulty=5, kill=True,
            start_time=0, end_time=1000, encounter_id=1,
            friendly_player_ids=[1, 10],  # Benchwarmer (20) not in this pull
        )
        parsed_fight = ParsedFight(fight=fight, actors=actors_with_sub, abilities={}, events=[])

        fight_roster = roster.get_fight_roster(parsed_fight)
        self.assertEqual(set(fight_roster.keys()), {1, 10})
        self.assertNotIn(20, fight_roster)

    def test_still_excludes_pets_even_if_listed(self):
        fight = Fight(
            id=1, name="Test Boss", difficulty=5, kill=True,
            start_time=0, end_time=1000, encounter_id=1,
            friendly_player_ids=[1, 10, 50],  # 50 is a pet, shouldn't happen but guard anyway
        )
        parsed_fight = ParsedFight(fight=fight, actors=ACTORS, abilities={}, events=[])
        fight_roster = roster.get_fight_roster(parsed_fight)
        self.assertNotIn(50, fight_roster)

    def test_empty_friendly_player_ids_returns_empty_roster(self):
        fight = Fight(
            id=1, name="Test Boss", difficulty=5, kill=True,
            start_time=0, end_time=1000, encounter_id=1,
            friendly_player_ids=[],
        ...
        parsed_fight = ParsedFight(fight=fight, actors=ACTORS, abilities={}, events=[])
        self.assertEqual(roster.get_fight_roster(parsed_fight), {})
