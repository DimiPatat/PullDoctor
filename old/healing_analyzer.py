...
Analyzes healing events within a parsed fight: per-healer output (total
effective healing, overheal, HPS, breakdown by spell) and per-target
output (who received how much healing, and from whom).

Pure function over a ParsedFight -- no network, no dependency on other
analyzer modules.

IMPORTANT re: WCL's healing event fields (matches Blizzard's own combat
log semantics): `amount` is already the EFFECTIVE healing done (HP
actually restored); `overheal` is a SEPARATE, additive figure for the
portion that was wasted on top of that. So:
    effective healing   = amount              (NOT amount - overheal)
    raw healing attempt = amount + overheal
    overheal %          = overheal / (amount + overheal)

Also: pet/summon actors (e.g. a Warlock's Wild Imp, a DK's Rune Weapon)
can land heals in some specs. Those are attributed back to the owning
player via Actor.owner_id, so a "healer" list only ever shows players.
"""
...
class HealerSummary:
    ...
    healing_by_ability: dict[str, int] = field(default_factory=dict)  # effective healing per spell

    @property
    def total_raw_healing(self) -> int:
        """Total healing attempted, including the wasted (overheal) portion."""
    ...
    def overheal_percent(self) -> float:
        ...
        if raw == 0:
            return 0.0
        return (self.total_overheal / raw) * 100

    def hps(self, fight_duration_ms: int) -> float:
        if fight_duration_ms <= 0:
            return 0.0
        return self.total_effective_healing / (fight_duration_ms / 1000)
...
class TargetHealingSummary:
    ...
    healing_by_source: dict[str, int] = field(default_factory=dict)  # effective healing per healer name
...
def analyze_healing(parsed_fight: ParsedFight) -> list[HealerSummary]:
    """
    Build a HealerSummary per player who cast (or whose pet cast) any
    heals in the fight, sorted by effective healing descending.
    """
    ...
    for event in parsed_fight.events:
        ...
        summary.healing_by_ability[ability_name] = (
            summary.healing_by_ability.get(ability_name, 0) + effective
        )
...
def analyze_healing_received(parsed_fight: ParsedFight) -> list[TargetHealingSummary]:
    """
    Build a TargetHealingSummary per actor who received any healing in
    the fight, sorted by total effective healing received descending.
    Healing sources are resolved through pets back to their owning
    player, same as analyze_healing.
    """
    ...
    for event in parsed_fight.events:
        ...
        if target_id not in summaries:
            summaries[target_id] = TargetHealingSummary(
        ...
        _, source_name = roster.resolve_to_player_id_name(
            event.source_id, event.source_name, parsed_fight.actors
        )
        source_name = source_name or "Unknown"
        summary.healing_by_source[source_name] = (
            summary.healing_by_source.get(source_name, 0) + effective
        )

    return sorted(
        summaries.values(),
        key=lambda s: s.total_effective_healing_received,
        reverse=True,
    )
...
def summarize_healing(parsed_fight: ParsedFight, summaries: list[HealerSummary]) -> str:
    ...
    for summary in summaries:
        lines.append(
            f"  {summary.healer_name or 'Unknown':<15} "
