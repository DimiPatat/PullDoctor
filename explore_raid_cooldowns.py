"""
explore_raid_cooldowns.py
Fetches ONE real fight from a Warcraft Logs report and runs
raid_cooldown_explorer.py's heuristic candidate-discovery scan against
it, printing anything found so you can decide what to track via
`manage_raid_cooldowns.py add`.

Only needs the Casts event stream -- no Healing/Damage/CombatantInfo
events are fetched here, keeping this fast and light on API usage.

Usage:
    python explore_raid_cooldowns.py <report_code> [fight_id]
"""
import argparse
import sys
...
import log_parser
import cli_helpers
import raid_cooldown_explorer
from raid_cooldown_config_io import load_generated_raid_cooldowns
...
def build_argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Scan one fight's Casts events for likely raid-cooldown candidates."
    )
...
def main():
    ...
    candidates = raid_cooldown_explorer.discover_candidate_raid_cooldowns(parsed, already_tracked_names)
    print(raid_cooldown_explorer.format_candidates(candidates))
